#!/usr/bin/env python3
"""
USCIS Form Instructions — RAG Ingestion
Reads the JSON output of scrape_form_instructions.py and loads each
section into rag_chunks (corpus='form_instructions').

Usage:
    python3 ingest_form_instructions.py --ingest          # parse JSON, upsert raw chunks
    python3 ingest_form_instructions.py --embed           # embed chunks missing vectors
    python3 ingest_form_instructions.py --ingest --embed  # both in one pass
    python3 ingest_form_instructions.py --status          # show chunk/embedding counts
    python3 ingest_form_instructions.py --reset --ingest  # wipe corpus and re-ingest
    python3 ingest_form_instructions.py --ingest --limit 3  # test with 3 records

Reads:
    FORM_INSTRUCTIONS_JSON env var  (default: perm-research/data/form_instructions.json)

Writes:
    rag_chunks (corpus='form_instructions') in perm_decisions database
"""
...
OVERLAP_TOKENS = 80
BATCH_SIZE   = 5
CORPUS       = "form_instructions"
DOC_INSTRUCT = ""

DEFAULT_JSON = Path(__file__).resolve().parent / "data" / "form_instructions.json"
JSON_PATH    = Path(os.environ.get("FORM_INSTRUCTIONS_JSON", str(DEFAULT_JSON)))
...
def run_ingest(conn, limit: int | None):
    if not JSON_PATH.exists():
        ...
        log.error("Run scrape_form_instructions.py first to generate it.")
        sys.exit(1)
    ...
    for rec in records:
        ...
        source_label = f"{form} Instructions — {classification}"
...
def main():
    parser = argparse.ArgumentParser(
        description=f"Ingest USCIS form instructions into RAG (corpus='{CORPUS}')"
    )
    ...
    parser.add_argument("--json",       default=None,
                        help=f"Path to instructions JSON (default: {DEFAULT_JSON})")
